...
def build_guided_summary_markdown(state: dict) -> str:
    ...
    lines = [
        ...
        "## AlphaFold / PPI",
        "",
        f"- AF3 candidate pairs: {state.get('af3_pair_count') or 0}",
        f"- AF3 JSON exported: {'yes' if state.get('af3_json_exported') else 'no'}",
        f"- AF3 JSON path: {state.get('af3_json_path') or 'not exported'}",
        f"- AF3 results folder: {state.get('af3_results_folder') or 'not selected'}",
        "",
        "## DaVinci / HPC",
        "",
